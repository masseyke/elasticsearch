...
public class FetchHealthInfoCacheActionTests extends ESTestCase {
    ...
    public void testAction() throws ExecutionException, InterruptedException {
        ...
        assertThat(
            actualResponse.getHealthNodeInfo(TestHealthNodeInfo.class),
            equalTo(expectedResponse.getHealthNodeInfo(TestHealthNodeInfo.class))
        );
    }
    ...
    private HealthInfoCache getTestHealthInfoCache() {
        ...
        for (DiscoveryNode allNode : allNodes) {
            ...
            healthInfoCache.updateNodeHealth(nodeId, new TestHealthNodeInfo(nodeId));
        }
    ...
    public void testResponseSerialization() {
        FetchHealthInfoCacheAction.Response response = new FetchHealthInfoCacheAction.Response(getTestHealthInfoCache().getHealthInfo());
    ...
    private FetchHealthInfoCacheAction.Response mutateResponse(FetchHealthInfoCacheAction.Response originalResponse) {
        Map<String, DiskHealthInfo> diskHealthInfoMap = originalResponse.getHealthNodeInfo(DiskHealthInfo.class);
        ...
        return new FetchHealthInfoCacheAction.Response(
            Map.of(
                DiskHealthInfo.class,
                diskHealthInfoMapCopy,
                TestHealthNodeInfo.class,
                originalResponse.getHealthNodeInfo(TestHealthNodeInfo.class)
            )
        );
    }
}
